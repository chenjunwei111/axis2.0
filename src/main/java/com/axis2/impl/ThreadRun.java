    ...
    public String getComMsg(String sendSheet, String chhUrl) {
        try {
            if(sendSheet!=null){
                ...
                //169库同时入进去
                getEmosComMsg169(sendSheet);
            }
            if(chhUrl!=null){
                getFtpMsg(chhUrl.replace("\n", ""),null);
                //ftp推送的CCH占用小区也入库到169
                getFtpMsg169(chhUrl.replace("\n", ""),null);
            }
            return "0";
        } catch (Exception e) {
            log.error("*******************************报文错误：\n", e);
...
//               System.out.println(map2);
    ...
    public void getEmosComMsg169(String sendSheet){
        try {
            //解析报文
            List<Map<String, Object>> listTotal = XmlUtil.xmlElementsList(sendSheet.replace("\n", ""));
            log.info("169报文解析开始。。。。。");
            ...
            for (Map<String, Object> map : listTotal) {
                ...
                ResultSet rs = JdbcServer169.executeStatement(checkSql);

                while (rs.next()) {
                    if (rs.getString(1) == null) {
                        log.info("169数据库报文工单不存在：" + msgId + ",执行下一步");
                    } else {
                        log.info("169数据库报文工单已存在：" + msgId + ",终止执行下一步");
                        orderType = "重复工单";
                    }
                }
                ...
                String cityCode = null;
                String checkCityCode = "select CITY_CODE from department_level where CITY_NAME='" + city + "' and rownum=1  ";
                ResultSet rs2 = JdbcServer169.executeStatement(checkCityCode);
                while (rs2.next()) {
                    cityCode = rs2.getString(1);
                }
                ...
                if (list != null && list.size() != 0 && list.get(0).getPoint() != null) {
                    ...
                    String checkProprtysql = "select DEPT_NAME From department\n" +
                            "where instr('"+web.getArea()+"',REGION_NAME)>0\n" +
                            "and PARENT_DEPT_CODE!='YUNNAN' ";
                    ResultSet rs3 = JdbcServer169.executeStatement(checkProprtysql);
                    while (rs3.next()) {
                        sProperty = rs3.getString(1);
                    }
...
//               System.out.println(map2);
            ...
            boolean res1 = JdbcServer169.inserts(insertLogSql, tempList);
            boolean res2 = JdbcServer169.inserts(insertComSql, tempList2);

            log.info(resposeMsg);
        }
        catch (Exception e) {
            log.error("*******************************169数据库工单报文接收错误：",e);
        }
    }
...
//            String pass = ConfigReaderUtils.getProperty("ftpPassWord");
     ...
     * @return
     */
    public String getFtpMsg169(String sendSheet, String orderNum) {
        XFtp ftp = null;
        Connection conn = null;
        log.info("开始解析ftp报文到169数据库。。。。。");
...
//            String pass = ConfigReaderUtils.getProperty("ftpPassWord");
            ...
            for (Map<String, Object> map : msgList) {
                ...
                log.info("遍历完成，开始入库169。。。。。");

                JdbcServer169.createDbLinkDatePartition("P_ACCEPT_CHH_INFO", time.substring(0, 10), 0);
                log.info("P_ACCEPT_CHH_INFO ,169数据库（" + time.substring(0, 10) + "）分区创建成功。。。。。");
                ...
                log.info("数据遍历完成，数量（" + tempList2.size() + "）开始入库169。。。。。");
                resposeMsg += fileName + " 文件入库169完成 \n";

            }
            JdbcServer169.inserts(insertLogSql, tempList);
            JdbcServer169.inserts(insertChhSql, tempList2);
            log.info(resposeMsg);
            return resposeMsg;
        } catch (Exception e) {
            log.error("*******************************169FTP报文错误：\n", e);
            return "error";
        } finally {
            if (conn != null) {
                conn.disConnectServer();
